...
#[quickcheck]
fn ordering_property(bt: BinaryTree<'static, i32, (i8, i8)>) -> bool {
    match bt {
...
#[quickcheck]
fn height_is_maintained(bt: AvlTree<'static, i32>) -> bool {
    match bt {
        BinaryTree {
...
#[quickcheck]
fn balance_property(bt: BinaryTree<'static, i32, (i8, i8)>) -> bool {
    match bt {
...
#[derive(Debug,Clone)]
struct BinaryTree<'a, V: 'a+Ord+Copy, M: 'a> {
        metadata: M,
        value: V,
        left: &'a Option<Box<BinaryTree<'a, V, M>>>,
        right: &'a Option<Box<BinaryTree<'a, V, M>>>
}


impl Arbitrary for BinaryTree<'static, i32, (i8, i8)> {
...
impl <'a, V: Ord+Copy+Clone+Send, M: Copy+Clone+Send> BinaryTree<'a, V, M> {
...
struct BinaryTreeIterator<'a, V: 'a+Ord+Copy+Clone+Send, M: 'a+Copy+Clone+Send> {
    to_visit: Vec<&'a BinaryTree<'a, V, M>>
}

impl <'a, V: 'a+Ord+Copy+Clone+Send, M: 'a+Copy+Clone+Send> Iterator for BinaryTreeIterator<'a, V, M> {
    type Item = &'a BinaryTree<'a, V, M>;

    fn next(&mut self) -> Option<&'a BinaryTree<'a, V, M>> {
...
type AvlTree<'a, V: 'a> = BinaryTree<'a, V, (i8, i8)>;


impl <'a, V: Debug+Ord+Copy> AvlTree<'a, V> {
    ...
    fn rotate_right(&mut self) {
        ...
        mem::swap(&mut self.left, &mut left.right);
        tmp.fix_metadata();
